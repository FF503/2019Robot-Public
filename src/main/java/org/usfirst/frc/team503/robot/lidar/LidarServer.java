...
/**
 * Starts the <code>chezy_lidar</code> C++ program, parses its output, and feeds
 * the LIDAR points to the {@link LidarProcessor}.
 * <p>
 * Once started, a separate thread reads the stdout of the
 * <code>chezy_lidar</code> process and parses the (angle, distance) values in
 * each line. Each resulting {@link LidarPoint} is passed to
 * {@link LidarProcessor.addPoint(...)}.
 */
public class LidarServer {
    ...
    public synchronized double getDistance(int angle) {
        double distance =  mLidarProcessor.getLidarPoint(angle);
        // System.out.println("Angle " + angle+ ", Distance " + distance);
        // SmartDashboard.putNumber("Lidar at " + angle, distance);
        return distance;
    }
