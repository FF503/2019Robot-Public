...
import org.usfirst.frc.team503.robot.utils.MotionMagicPID;
...
public class Wrist extends Subsystem implements SuperStructureSystem {
  ...
  public Wrist() {
    if (!Robot.bot.getName().equals("ProgrammingBot")) {
      ...
      wristLimit = new DigitalInput(Robot.bot.wristLimID);
      wristMotor.configSelectedFeedbackSensor(
          /* FeedbackDevice.CTRE_MagEncoder_Relative */ FeedbackDevice.CTRE_MagEncoder_Absolute, Robot.bot.gSlotIdx,
  ...
  private void initListener() {
    table.putBoolean("Reset Encoder", false);
    table.getEntry("Reset Encoder").addListener(event -> {
      boolean bool = event.getEntry().getBoolean(false);
      if (bool && DriverStation.getInstance().isDisabled()) {
        resetEncoder();
  ...
  public void sendDashboardData() {
    ...
    table.putBoolean("Wrist Limit", getWristLimit());
    if (Robot.bot.hasWrist()) {
